...
def test_migrate_plugins_security_profile_when_local_plugins_exist(tmp_path: Path, monkeypatch):
    ...
    assert cfg.get("plugins.directory_scan_auto_migrated") is True

    saved = yaml.safe_load(cfg_file.read_text(encoding="utf-8"))
    assert saved["plugins"]["security_profile"] == "development"
    assert saved["plugins"]["directory_scan_auto_migrated"] is True

    Config._instance = None


def test_migrate_plugins_security_profile_runs_once(tmp_path: Path, monkeypatch):
    project = tmp_path / "proj"
    project.mkdir()
    local = project / "plugins" / "local" / "demo_plugin"
    local.mkdir(parents=True)
        ...
        yaml.safe_dump(
            {
                "plugins": {
                    ...
                    "modules": [],
                    "directory_scan_auto_migrated": True,
                }
            }
        ),
        encoding="utf-8",
    ...
    Config._instance = None
    cfg = Config()
    assert cfg.get("plugins.security_profile") == "production"
