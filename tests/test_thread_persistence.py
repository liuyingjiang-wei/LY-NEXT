...
@pytest.mark.asyncio
async def test_resolve_external_telegram_thread_id(monkeypatch):
    from ly_next.core import thread_persistence as tp
    ...
    key = "telegram:chat:6537629878:user:6537629878"
    tid = await tp.resolve_thread_id(key, seed_messages=[{"role": "user", "content": "hi"}])
    assert tid == str(session_id)
    assert stored_key == [key]


def test_is_external_thread_key_accepts_telegram():
    from ly_next.core.thread_persistence import is_external_thread_key

    assert is_external_thread_key("telegram:chat:1:user:1")
    assert is_external_thread_key("onebot:private:123")
    assert not is_external_thread_key("not-a-valid-key")
    assert not is_external_thread_key("123e4567-e89b-12d3-a456-426614174000")
