...
async def test_prepare_runs_vision_and_routing_in_parallel():
    ...
    with (
        patch(
            "ly_next.agent.chat_pipeline.prepare_messages_for_agent", new_callable=AsyncMock
        ) as prep,
        patch(
            "ly_next.agent.chat_pipeline.messages_need_vision_precaption",
            return_value=True,
        ),
        patch(
...
@pytest.mark.asyncio
async def test_prepare_overlaps_augment_and_routing_without_vision():
    calls: list[str] = []

    async def fake_aug(messages, **kwargs):
    ...
    with (
        ...
        await prepare_chat_turn(
            ChatTurnRequest(
                client_messages=[{"role": "user", "content": "Explain useEffect in React"}],
                parallel_prep=True,
                persist_user_async=False,
            )
        )

    assert "augment" in calls and "route" in calls
    assert calls.index("augment") < 2 and calls.index("route") < 2
