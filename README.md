...
sequenceDiagram
  ...
  Note over P: 持久化用户消息 · RAG/记忆增强 · 识图预描述 · 默认模型解析
...
| **Chat Pipeline** | 对话前的统一入口：消息准备、增强、默认模型选择与 Agent 调度 |
| **消息桥接** | QQ / Telegram 等以 `plugins/` 插件形式挂载；WS 桥接在 `create_app` 早期注册 |
...
QQ / Telegram 等以 **独立 Git 仓库** 维护，安装到 `plugins/local/` 或通过 pip，**不随 core 提交**。
...
配置键：`bridge.onebot11.*`、`bridge.telegram.*`。安装与开发流程见 **[plugins/README.md](./plugins/README.md)**；各插件仓库内自带 README。
