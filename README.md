...
默认启用 API Key（请求头 `X-API-Key` 或登录 Cookie）。工作台侧栏 **「应用配置」** 与 **「模型配置」** 均通过 `GET/PATCH /api/system/settings` 读写同一配置文件（可编辑字段以接口白名单为准；密钥回显为 `***`）。MCP 相关在 **「MCP」** 页。自检：`GET /api/system/extensions`。
